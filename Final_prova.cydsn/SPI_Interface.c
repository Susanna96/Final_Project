...
uint8_t SPI_Interface_ReadByte(uint8_t byteTX) {
        ...
        /* Read the RX Buffer */
        SPIM_ReadRxData();  //prima lettura corrisponde alla fase di scrittura del dummy byte
        uint8_t byteRX = SPIM_ReadRxData(); //lettura secondo byte che ci interessa (FIFO)
