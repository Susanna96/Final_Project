...
int main(void)
{
    ...
    /* When the device is turned on the state is STOP and the parameters (FSR and previous state) are read from EEPROM */
    int state=STOP;
    Parameters=EEPROM_readByte(0x0000);
    
    // 4-bit right shift to remove the previous state and keep only the bits that encode for the FSR
    ...
    // Save also the state in the variable Parameters
    ...
    {                       
        ...
        switch (state)
        {
            ...
            /* Read FIFO_SRC register to check for overrun event */
            error = I2C_Peripheral_ReadRegister(LIS3DH_DEVICE_ADDRESS,
                                         LIS3DH_FIFO_SRC_REG,
                                         &fifo_src_reg); 
