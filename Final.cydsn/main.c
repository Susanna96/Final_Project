...
/* SENSITIVITY DEVE ESSERE UNA VARIABILE CHE CAMBIA !!!!! */

//Brief value of sensitivity in High Resolution mode and FS=4g (2 mg/digit)
#define HR_SENSITIVITY_4g 2
...
int main(void)
{
    ...
    if (ctrl_reg4 != LIS3DH_CTRL_REG4_4g)
    {
    ...
    /* ----------------------------------------
			ACQUISITION OF OUTPUT ACCELERATION 
			DATA THROUGH LIS3DH FIFO BUFFER 
		--------------------------------------- */
         ...
         //if (INT_pin_Read()==1)
            {
                ...
                if (error == NO_ERROR)
                {
                    ...
                    X_Out[i]=abs(X_Out[i]*HR_SENSITIVITY_4g);
                    ...
                    /*scaling to mg (sensitivity=2mg/digit)*/
                    Y_Out[i]=abs(Y_Out[i]*HR_SENSITIVITY_4g);
                    ...
                    /*scaling to mg (sensitivity=2mg/digit)*/
                    Z_Out[i]=abs(Z_Out[i]*HR_SENSITIVITY_4g);
                    
                    // Change blinking frequency of RGB LED according to acc. values
